...
def mysql_connection(mysql_database, mysql_user, mysql_user_password):
    ...
    connection_config = 'mysql+pymysql://{0}:{1}@{2}:{3}/{4}'.format(
        mysql_user, mysql_user_password, host, port, mysql_database)
...
def write_to_mysql(mysqlConnection, table_name, user_sessions_spDF):
    ...
    user_sessions_df.to_sql(
        con=mysqlConnection, name=table_name, if_exists='append', index=False)
...
def main():
    ...
    for user_sessions_chunk_df in user_sessions_chunks_df:
        ...
        logging.info(
            'Loading DF Data from the Batch into batch_data MySQL Table')
